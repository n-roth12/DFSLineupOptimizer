...
def optimize_lineup(mode: str, filepath: str):
    ...
    lineup_builder = LineupBuilder(SITES[site].MODES.get(mode).get("POSITIONS"),
                                   SITES[site].NAME,
    ...
    for i in range(300):
        ...
        if salary <= SITES[site].MODES.get(mode).get("SALARY_CAP") and proj_points > best_proj_points:
