...
from .fanduel_draftables import fanduel_draftables
from lineup_optimizer.lineup import Lineup


class FanduelLineupBuilderTests(unittest.TestCase):

    def test_lineup_builder_get(self):
        lineup_builder = LineupBuilder(mode="FULL_ROSTER",
                                       site="DRAFTKINGS",
                                       draftables=fanduel_draftables)
    ...
    def test_with_valid_composition_rule(self):
        lineup_builder = self.default_fanduel_builder().with_composition_rule(
    ...
    def test_with_invalid_composition_rule(self):
        self.assertIsNone(self.default_fanduel_builder().with_composition_rule(
    ...
    def test_with_valid_stack_rule_3_1(self):
        ...
        self.assertEqual(lineup_builder.get("WR3").eligible_teams, ["KC"])
        self.assertFalse(["KC"] in lineup_builder.get("FLEX").eligible_teams)
    ...
    def test_pick_valid_player_3(self):
        lineup_builder = self.default_fanduel_builder()
        player = lineup_builder.pick_player(position="RB")
        self.assertEqual("RB", player.get("position"))

    def test_pick_invalid_player(self):
        self.assertIsNone(self.default_fanduel_builder().pick_player(
            position="WR", eligible_teams=["KI"], max_salary=4000))

    def test_build(self):
        lineup = self.default_fanduel_builder().build()
    ...
    def test_build_1(self):
        ...
        self.assertEqual("KC", lineup.get("WR3")["team_abbr"])
    ...
    # # this has a small chance of failing (~ 1 in 5^47 for full roster) of the time
    # # because optimize() is not deterministic, it is not gauranteed that a lineup
    # # under the salary cap will be found within 100 tries
